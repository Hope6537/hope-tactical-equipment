...
public class EventController {
    ...
    @ResponseBody
    public Response postEventData(HttpServletRequest request, @RequestBody String receiveData) {
        //获取设备信息
        try {
            ...
            //--公共部分完成--
            EventDto eventDto = dataMap.getObject("postObject", EventDto.class);
            //判断eventDto是否合法,还需要校验其他字段
            checkNotNull(eventDto, ResponseDict.ILLEGAL_REQUEST);
            ResultSupport<Integer> operationResult = eventService.addEvent(eventDto);
    ...
    @ResponseBody
    public Response putEventData(HttpServletRequest request, @RequestBody String receiveData) {
        try {
            checkNotNull(receiveData, ResponseDict.ILLEGAL_PARAM);
            JSONObject dataMap = (JSONObject) request.getAttribute("dataMap");
            if (dataMap == null) {
            ...
            EventDto eventDto = dataMap.getObject("putObject", EventDto.class);
            //判断eventDto是否合法,还需要校验其他字段
            checkNotNull(eventDto, ResponseDict.ILLEGAL_REQUEST);
            checkNotNull(eventDto.getId(), ResponseDict.ILLEGAL_REQUEST);
            ResultSupport<Integer> operationResult = eventService.modifyEvent(eventDto);
            return Response.getInstance(operationResult.isSuccess()).addAttribute("result", operationResult.getModule());
        } catch (JSONException jsonException) {
            return Response.getInstance(false).setReturnMsg(ResponseDict.ILLEGAL_PARAM);
        } catch (Exception e) {
    ...
    @ResponseBody
    public Response deleteEventData(HttpServletRequest request, @RequestBody String receiveData) {
        try {
            checkNotNull(receiveData, ResponseDict.ILLEGAL_PARAM);
            JSONObject dataMap = (JSONObject) request.getAttribute("dataMap");
            if (dataMap == null) {
            ...
            EventDto eventDto = dataMap.getObject("deleteObject", EventDto.class);
            //判断eventDto是否合法,还需要校验其他字段
            checkNotNull(eventDto, ResponseDict.ILLEGAL_REQUEST);
            checkNotNull(eventDto.getId(), ResponseDict.ILLEGAL_REQUEST);
            ResultSupport<Integer> operationResult = eventService.removeEvent(eventDto.getId());
            return Response.getInstance(operationResult.isSuccess()).addAttribute("result", operationResult.getModule());
        } catch (JSONException jsonException) {
            return Response.getInstance(false).setReturnMsg(ResponseDict.ILLEGAL_PARAM);
        } catch (Exception e) {
    ...
    @ResponseBody
    public Response getEventData(HttpServletRequest request) {
        try {
            JSONObject dataMap = (JSONObject) request.getAttribute("dataMap");
            if (dataMap == null) {
                Object errorResponse = request.getAttribute("errorResponse");
            ...
            EventDto eventDto = dataMap.getObject("event", EventDto.class);
            //判断eventDto是否合法,还需要校验其他字段
            checkNotNull(eventDto, ResponseDict.ILLEGAL_REQUEST);
            checkNotNull(eventDto.getId(), ResponseDict.ILLEGAL_REQUEST);
            ResultSupport<EventDto> result = eventService.getEventById(eventDto.getId());
    ...
    @ResponseBody
    public Response fetchEventData(HttpServletRequest request) {
        try {
            JSONObject dataMap = (JSONObject) request.getAttribute("dataMap");
            if (dataMap == null) {
                Object errorResponse = request.getAttribute("errorResponse");
