...
public class PublishServiceImpl implements PublishService {
    ...
    @Override
    public ResultSupport<List<PublishDto>> getPublishListByEventIdList(List<Integer> idList) {
        boolean flag;
        List<PublishDto> result;
        List<PublishDo> disableResultList;
        try {
            checkNotNull(idList, "[批量查询失败][当前入参实体为空]");
            List<PublishDo> list = publishDao.selectPublishListByEventIds(idList);
            checkNotNull(list, "[批量查询失败][查询为空]");
            disableResultList = list.parallelStream().filter(o -> o.getId() == null || o.getStatus() == null || o.getIsDeleted() == null).collect(Collectors.toList());
            if (disableResultList == null) {
                disableResultList = Lists.newArrayList();
    ...
    @Override
    public ResultSupport<List<PublishDto>> getPublishListByClassesIdList(List<Integer> idList) {
        boolean flag;
        List<PublishDto> result;
        List<PublishDo> disableResultList;
        try {
            checkNotNull(idList, "[批量查询失败][当前入参实体为空]");
            List<PublishDo> list = publishDao.selectPublishListByClassesIds(idList);
            checkNotNull(list, "[批量查询失败][查询为空]");
            disableResultList = list.parallelStream().filter(o -> o.getId() == null || o.getStatus() == null || o.getIsDeleted() == null).collect(Collectors.toList());
            if (disableResultList == null) {
                disableResultList = Lists.newArrayList();
