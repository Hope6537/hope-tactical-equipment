...
public class ClassesServiceImpl implements ClassesService {
    ...
    public ResultSupport<Integer> addClasses(ClassesDto classesDto) {
        ...
        Integer id;
        try {
            checkNotNull(classesDto, "[添加失败][当前插入数据实体为空]");
            if (classesDto.getStatus() == null) {
                classesDto.setStatus(0);
            }
            ClassesDo classesDo = mappingConverter.doMap(classesDto, ClassesDo.class);
            result = classesDao.insertClasses(classesDo);
            checkNotNull(classesDo.getId(), "[添加失败][数据库没有返回实体ID]");
            id = classesDo.getId();
        } catch (Exception e) {
    ...
    @Override
    public ResultSupport<Integer> addClasses(String name) {
        try {
        ...
        return this.addClasses(new ClassesDto(name));
    }

    @Override
    public ResultSupport<Integer> modifyClasses(ClassesDto classesDto) {
        Integer result;
        try {
            checkNotNull(classesDto, "[单体更新失败][当前入参实体为空]");
            checkNotNull(classesDto.getId(), "[单体更新失败][当前入参实体ID为空,无法更新]");
            result = classesDao.updateClasses(mappingConverter.doMap(classesDto, ClassesDo.class));
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return ResultSupport.getInstance(e);
        }
    ...
    @Override
    public ResultSupport<Integer> batchModifyClasses(ClassesDto classesDto, List<Integer> idList) {
        Integer result;
        try {
            checkNotNull(classesDto, "[批量更新失败][当前入参实体为空]");
            checkNotNull(idList, "[批量更新失败][当前入参实体ID为空,无法更新]");
            checkArgument(idList.size() > 0, "[批量更新失败][当前入参实体更新的ID集合大小为0]");
            result = classesDao.batchUpdateClasses(mappingConverter.doMap(classesDto, ClassesDo.class), idList);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return ResultSupport.getInstance(e);
        }
    ...
    @Override
    public ResultSupport<Integer> removeClasses(Integer id) {
        Integer result;
        try {
            checkNotNull(id, "[单体删除失败][当前入参实体为空]");
            ClassesDo readyToDelete = classesDao.selectClassesById(id);
            checkNotNull(readyToDelete, "[单体删除失败][ID=" + id + "记录从未存在]");
            checkArgument(readyToDelete.getIsDeleted() != 1, "[单体删除失败][ID=" + id + "记录已经被删除]");
            result = classesDao.deleteClasses(id);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return ResultSupport.getInstance(e);
        }
    ...
    @Override
    public ResultSupport<Integer> batchRemoveClasses(List<Integer> idList) {
        Integer result; //返回结果
        ...
        try {
            ...
            List<ClassesDo> readyToDelete = classesDao.selectClassesListByIds(idList);
            ...
            result = classesDao.batchDeleteClasses(idList);
    ...
    @Override
    public ResultSupport<ClassesDto> getClassesById(Integer id) {
        ClassesDto result;
        try {
            checkNotNull(id, "[单体查询失败][当前入参实体为空]");
            ClassesDo comicDo = classesDao.selectClassesById(id);
            result = mappingConverter.doMap(comicDo, ClassesDto.class);
            //判断单条数据是否合法
    ...
    @Override
    public ResultSupport<List<ClassesDto>> getClassesListByIdList(List<Integer> idList) {
        boolean flag;
        List<ClassesDto> result;
        List<ClassesDo> disableResultList;
        try {
            checkNotNull(idList, "[批量查询失败][当前入参实体为空]");
            List<ClassesDo> list = classesDao.selectClassesListByIds(idList);
            ...
            result = list.parallelStream().filter(o -> o.getId() != null).map(o -> mappingConverter.doMap(o, ClassesDto.class)).collect(Collectors.toList());
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return ResultSupport.getInstance(e);
        }
    ...
    @Override
    public ResultSupport<List<ClassesDto>> getClassesListByQuery(ClassesDto query) {
        List<ClassesDto> result;
        ...
        try {
            ...
            ClassesDo doQuery = mappingConverter.doMap(query, ClassesDo.class);
            countByQuery = classesDao.selectClassesCountByQuery(doQuery);
            result = classesDao.selectClassesListByQuery(doQuery).stream().map(o -> mappingConverter.doMap(o, ClassesDto.class)).collect(Collectors.toList());
        ...
        ResultSupport<List<ClassesDto>> instance = ResultSupport.getInstance(expr, expr ? "[条件查询成功]" : "[条件查询失败][查询无数据,请变更查询条件]", result);
